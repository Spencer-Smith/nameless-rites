...
$(document).ready(function() {
  ...
  var formatTrelloList = function(cards, listId) {
    var results = "";
    for (var j = 0; j < cards.length; j++) {
      if (cards[j].idList != listId)
        continue;
  ...
  var formatTrelloBoard = function(lists) {
    ...
    for (var i = 0; i < lists.length; i++) {
      ...
      results += "<div class=\"trellolist\">";
      results += "<h3>" + lists[i].name + "</h3>";
      results += formatTrelloList(cards, lists[i].id);
      results += "</div>";
      results += "</div>";
    }
    results += "</div>";
  ...
  var cssFormatting = function(numLists) {
    ...
    $(".griditem").css("width", (100/numLists).toString() + "%");
  ...
  Trello.get("board/"+boardId+"/cards", function(result) {
    cards = result;
    Trello.get("board/"+boardId+"/lists", function(lists) {
      formatTrelloBoard(lists);
      cssFormatting(lists.length);
    });
  });
